...
const ReferralDashboard: React.FC = () => {
    ...
    if (error) {
      ...
      if (isAuthError) {
        ...
        if (!accessToken || !profileToken) {
          console.warn('Missing authentication tokens, user may need to log in again');
  ...
  const getMilestoneProgress = (currentLevel: number, successfulReferrals: number): number => {
    ...
    <div className="space-y-6">
      <div className="relative overflow-hidden rounded-lg bg-primary/5 px-8 pt-4 pb-4 mb-8">
        ...
        <div className="relative flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div>
            ...
            <p className="text-muted-foreground mt-2 text-lg">Grow your network and earn rewards</p>
          </div>
          <Button
            onClick={() => refetch()}
            variant="secondary"
      ...
      >
        <TabsList className="inline-flex p-1 bg-muted/50 backdrop-blur-sm rounded-lg border border-border/50">
          <TabsTrigger value="overview" className="px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
            Overview
          </TabsTrigger>
          <TabsTrigger value="tree" className="px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
            Referral Tree
          </TabsTrigger>
          <TabsTrigger value="leaderboard" className="px-6 py-2 text-sm font-medium transition-all data-[state=active]:bg-background data-[state=active]:text-primary data-[state=active]:shadow-sm">
            Leaderboard
          </TabsTrigger>
        </TabsList>
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
            ...
            <Card className="transition-all duration-200 hover:shadow-md bg-gradient-to-br from-background via-background to-primary/5">
              <CardHeader>
                ...
                <CardDescription>Track your referral milestone level</CardDescription>
              </CardHeader>
              <CardContent>
                {isLoading ? (
                  ...
                  <>
                    ...
                    <div className="flex items-center space-x-2 mb-4">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {getNextMilestoneRequirement(referralStats?.currentMilestoneLevel || 0)}
                      </p>
                    </div>
                    <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="flex items-center justify-between mb-6 bg-muted/50 rounded-lg p-4">
                      <div className="flex items-center gap-4">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">MyPts Earned</p>
                        </div>
                        <div className="border-l border-border pl-4">
                          <p className="text-xl font-semibold text-amber-500">{referralStats?.pendingPoints || 0}</p>
                          <p className="text-sm text-muted-foreground">Pending MyPts</p>
                        </div>
                      </div>
            ...
            <div className=" gap-6 w-full">
              ...
              <Card className="lg:col-span-2 transition-all duration-200 hover:shadow-md bg-gradient-to-br from-background via-background to-primary/5">
                ...
                <CardContent>
                  {isLoading ? (
                  ...
                  ) : referralStats?.referredProfiles && referralStats.referredProfiles.length > 0 ? (
                    <div className="space-y-4">
                      {referralStats.referredProfiles.map((referral) => (
                        ...
                        >
                          <div className="flex items-center space-x-3">
                            <Avatar>
                              <AvatarImage src={referral.profile.profileImage} />
                              <AvatarFallback>{referral.profile.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                            </Avatar>
                            <div>
                          ...
                          <Badge
                            variant={referral.hasReachedThreshold ? "default" : "outline"}
                          ...
                          >
                            {referral.hasReachedThreshold ? 'Qualified' : 'Pending'}
                          </Badge>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="text-center py-8 bg-muted/30 rounded-lg border border-dashed border-primary/20">
                      ...
                      <p className="text-muted-foreground font-medium">You haven't referred anyone yet</p>
                      <p className="text-sm text-muted-foreground/80 mt-1 mb-4">Share your referral code to start earning rewards</p>
                      <Button
                        variant="outline"
                        className="bg-background/50 hover:bg-primary/10 transition-all duration-200"
                        onClick={() => setIsShareModalOpen(true)}
                  ...
                  <CardContent>
                    <div className="flex items-center space-x-3">
                      <Avatar className="ring-2 ring-offset-2 ring-offset-background ring-amber-500/20 transition-all duration-200">
                        ...
                        <AvatarFallback className="bg-amber-500/10">{referralStats.referredBy.name.substring(0, 2).toUpperCase()}</AvatarFallback>
                      </Avatar>
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Your Referrer</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        >
          <Card className="transition-all duration-200 hover:shadow-md bg-gradient-to-br from-background via-background to-primary/5">
            <CardHeader>
              ...
              <CardDescription>Visualize your complete referral tree</CardDescription>
            </CardHeader>
            <CardContent>
              <ReferralTree />
            </CardContent>
        ...
        >
          <Card className="transition-all duration-200 hover:shadow-md bg-gradient-to-br from-primary/5 via-background to-background">
            <CardHeader>
              ...
              <CardDescription>Top referrers in the MyPts community</CardDescription>
            </CardHeader>
            <CardContent>
              <ReferralLeaderboard />
            </CardContent>
