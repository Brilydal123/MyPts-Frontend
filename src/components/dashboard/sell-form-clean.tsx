...
export function SellFormClean({ balance, onSuccess }: SellFormProps) {
  ...
  // Validate payment method details - memoized to prevent re-renders
  const validatePaymentMethod = (checkOnly = false): boolean => {
    ...
    // Only update state if we're not just checking
    if (!checkOnly) {
  ...
  // We'll check form validity only when needed, not during render
