...
export function SimpleSellForm({ balance, onSuccess }: SellFormProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (values.paymentMethod === 'bank_transfer') {
      ...
      if (!accountDetails.cardholderName?.trim() || 
      ...
      if (!accountDetails.cryptoType?.trim() || 
          !accountDetails.walletAddress?.trim()) {
        errorMessage = 'Please complete all cryptocurrency details';
      } else {
        isValid = true;
      }
    }
    ...
    if (!isValid) {
      toast.error('Incomplete payment information', {
        description: errorMessage || 'Please complete all required payment method fields before proceeding.',
      });
      return;
    }
